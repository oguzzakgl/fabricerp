...
const Invites: React.FC = () => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await superAdminApi.createInvite(newCode);
      alert('Davet kodu oluşturuldu.'); setModalOpen(false); setNewCode(''); load();
    } catch (e: unknown) {
          ...
          <tbody className="divide-y divide-outline-variant text-sm">
            {invites.map(i => (
              <tr key={i.id} className="hover:bg-arka-plan-gri/50 transition-colors">
                ...
                <td className="py-4 px-4 font-mono text-xs text-on-surface-variant">{i.usedByEmail || '-'}</td>
            ...
            {invites.length === 0 && (
              <tr><td colSpan={5} className="py-10 text-center text-sm text-on-surface-variant">Davet kodu bulunamadı.</td></tr>
            )}
          </tbody>
        </table>
      </div>
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              <div className="space-y-1">
                ...
                <input type="text" value={newCode} onChange={e => setNewCode(e.target.value.toUpperCase())}
                  ...
                  className="w-full px-3 py-2 bg-arka-plan-gri border border-outline-variant rounded font-mono uppercase tracking-wider text-sm outline-none focus:ring-1 focus:ring-bilgi-mavisi" />
              </div>
